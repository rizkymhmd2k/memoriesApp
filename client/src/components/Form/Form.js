...
const Form = ({currentId, setCurrentId}) => {
    ...
    const post = useSelector(state => currentId ? state.memoryPosts.posts.posts.find(p => p._id === currentId) : null)
    ...
    const handleSubmit = (e) => {
        ...
        if(currentId){
            ...
            dispatch(createPost({navigate, postData : {...postData, name : user?.result?.name}}))
           // navigate(`/posts/${_id}`), cant like this bcs id isnt specified
            clear();
        }
    ...
    return (
        <Paper elevation={2} sx={{ padding: '15px',     borderRadius: 3, }}>
